...
def test_discover_presets_code_and_issues(tmp_path):
    ...
    (user / "reshape").mkdir()
    (user / "reshape" / "swap.py").write_text(
        "from mapchar.plugins.base import PluginInfo, Stage\n"
        "class R:\n"
        "    info = PluginInfo('swap2', 'Swap', Stage.RESHAPE)\n"
        "    def reshape(self, data, ctx): return data[::-1]\n"
        "    def unshape(self, data, ctx): return data[::-1]\n"
        "def register(r): r.register(R())\n"
    )
    (user / "reshape" / "wrong_stage.py").write_text(
        "from mapchar.plugins.base import PluginInfo, Stage\n"
        "class C:\n"
    ...
    (user / "reshape" / "broken.py").write_text("raise RuntimeError('boom')\n")
    (user / "charsets").mkdir()
    ...
    assert reg.plugin(Stage.RESHAPE, "swap2").info.category == "Your plugins"
    assert list(reg.plugin(Stage.CHARSET, "mytbl").entries()) == [
        ("01000001", "X"),
        ("01000010", "Y"),
    ]
    messages = " ".join(str(i) for i in result.issues)
    assert "unknown engine" in messages and "not a reshape plugin" in messages
