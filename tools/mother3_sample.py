...
def _code_lines(code: int, label: str, words: int, note: str | None) -> list[str]:
    lines = [f"# {note}"] if note else []
    if code == 0xFF0B:
        return [*lines, f"!{_key(code)}=[{label}] @saturn:*"]
    if words:
        return [*lines, f"${_key(code)}=[{label}]" + ",u16" * words]
    # A page break reads best on a line of its own.
    return [*lines, f"{_key(code)}=[{label}]" + ("\\n" if code == 0xFF00 else "")]


def _script_codes(rom: bytes, saturn: bool) -> list[str]:
    counts = operand_counts(rom)
    counts[0xFF07] = 0  # used, and not in the table: the lookup's default
    lines = ["/FFFF=[end]"]
    for code in sorted(counts):
        if code == 0xFFFF or (saturn and code == 0xFF0B):
            continue
        label = SCRIPT_LABELS.get(code, f"FF{code & 0xFF:02X}")
        lines += _code_lines(code, label, counts[code], SCRIPT_NOTES.get(code))
    return lines


def table_files(rom: bytes) -> dict[str, str]:
    """The sample's three native tables, by file name."""
    chars = [
        f"{_key(i)}={_escape(text)}" for i, text in sorted(characters(rom).items())
    ]
    main = _header(
        "m3",
        "Mother 3 (Japan): u16 glyph indices into the Shift-JIS-ordered fonts at\n"
        "$CE39F8 and $D0B010, and the script's control codes with the operand\n"
        "counts the table at $D2DE58 gives them. Generated from the ROM by\n"
        "tools/mother3_sample.py.",
    )
    battle = _header(
        "m3battle",
        "Mother 3 (Japan) battle messages: the script's characters, and the\n"
        "codes the battle message loop at $080734A0 substitutes, none of which\n"
        "takes an operand. Generated from the ROM by tools/mother3_sample.py.",
    )
    saturn = _header(
        "saturn",
        "The Mr. Saturn font at $D1CE78, which [saturn] switches to for the\n"
        "rest of a string. Generated by tools/mother3_sample.py.",
    )
    saturn_chars = dict(enumerate(SATURN)) | SATURN_EXTRA
    return {
        "m3.tbl": "\n".join([*main, *_script_codes(rom, False), "", *chars]) + "\n",
        "m3battle.tbl": "\n".join(
            [
                *battle,
                "/FFFF=[end]",
                *(
                    f"{_key(c)}=[{'line' if c == 0xFF01 else f'FF{c & 0xFF:02X}'}]"
                    for c in BATTLE_CODES
                ),
                "",
                *chars,
            ]
