...
| `gbk`         | every code of the simplified Chinese double-byte encoding |
| `utf-16le`    | every plane as 16-bit keys, an astral code point as a surrogate pair (4 bytes) |
...
characters onto one code. `jis-x-0201` and `shift-jis` alias the JIS X 0201 yen
