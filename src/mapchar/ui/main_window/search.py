...
class SearchMixin:
    ...
    _find_needle: bytes | None = None
    """The bytes F3 and Shift+F3 walk; set by the last search that found any."""
    ...
    def _find_bytes(self, again: bool = False, backwards: bool = False) -> None:
        ...
        if self._doc is None:
            return
        if again and self._find_needle is not None:
            ...
            if not ok or not text.strip():
                return
            found = self._needle_from(text.strip())
            if found is None:
                return
            needle = found
    ...
    def _search_selection(self) -> None:
        if not self._selection or self._doc is None:
            return
        s, e = self._selection
        self._find_needle = self._doc.data[s:e]
