...
from mapchar.core.textmatch import next_match
...
class SearchMixin:
    ...
    def _next_match(self, needle: bytes, backwards: bool) -> int | None:
        ...
        *above* it, which would hand the next search the same match again.
        ...
        (:func:`~mapchar.core.textmatch.next_match`).
        """
        ...
        return next_match(
            ...
            lo,
            hi,
            self._selection[0] if self._selection else self._offset,
            backwards=backwards,
            on_match=bool(self._selection),
        )
